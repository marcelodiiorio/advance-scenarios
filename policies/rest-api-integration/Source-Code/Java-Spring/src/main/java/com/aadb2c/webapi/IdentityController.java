...
public class IdentityController {
    ...
    public ResponseEntity<B2CResponseModel> loyalty(@RequestBody(required = false) Map<String, String> body) {
        ...
        if (body.containsKey("language") == false || body.get("language").isEmpty()) {
            return ResponseEntity.status(HttpStatus.CONFLICT).body(new B2CResponseError("Language code is null or empty"));
        }

        // Check if the objectId parameter is provided
        if (body.containsKey("objectId") == false || body.get("objectId").isEmpty()) {
            return ResponseEntity.status(HttpStatus.CONFLICT).body(new B2CResponseError("User object Id is null or empty"));
        }

        return ResponseEntity.ok(new B2CResponse(body.get("language") + "-" + ((int )(Math.random() * 9999 + 1000)), null));
    }
    ...
    public ResponseEntity<B2CResponseModel> validate(@RequestBody(required = false) Map<String, String> body) {
        ...
        if (body.get("email").toLowerCase().startsWith("test")) {
            return ResponseEntity.status(HttpStatus.CONFLICT).body(new B2CResponseError("Your email address cannot start with 'test'"));
        }

        return ResponseEntity.ok(new B2CResponse(body.get("language") + "-" + ((int )(Math.random() * 9999 + 1000)), body.get("email").toLowerCase()));
    }
}